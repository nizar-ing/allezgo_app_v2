...
export default function EditTestimonialModal({ testimonial, onClose }) {
    ...
    useEffect(() => {
        if (testimonial && formRef.current) {
    ...
    const submitTestimonial = async (prevState, formData) => {
        try {
            ...
            await updateTestimonial({ id: testimonial.id, payload });
            handleClose();
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-sky-950/40 md:backdrop-blur-sm font-sans p-4">
            <div className="w-full max-w-lg bg-white rounded-xl shadow-xl flex flex-col overflow-hidden border border-sky-100">
                ...
                {/* Form Body */}
                <form ref={formRef} action={formAction} noValidate className="flex flex-col">
                    <div className="p-6">
                        {state?.error && (
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input type="text" name="name" defaultValue={testimonial.name} onKeyDown={preventEnterSubmit} required className="block w-full px-3 py-2 border border-sky-100 bg-slate-50/50 rounded-lg focus:ring-2 focus:ring-sky-500" placeholder="Ex: Jean Dupont" />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-slate-700 mb-1">URL de l'image (Avatar)</label>
                                <input type="url" name="imageUrl" defaultValue={testimonial.imageUrl} onKeyDown={preventEnterSubmit} required className="block w-full px-3 py-2 border border-sky-100 bg-slate-50/50 rounded-lg focus:ring-2 focus:ring-sky-500" placeholder="https://..." />
                            </div>
                            <div>
                                <label className="block text-sm font-semibold text-slate-700 mb-1">Citation (Témoignage)</label>
                                <textarea name="citation" defaultValue={testimonial.citation} rows="4" onKeyDown={preventEnterSubmit} required className="block w-full px-3 py-2 border border-sky-100 bg-slate-50/50 rounded-lg focus:ring-2 focus:ring-sky-500 resize-none" placeholder="Superbe expérience avec AllezGo..."></textarea>
                            </div>
                        </div>
                    </div>
